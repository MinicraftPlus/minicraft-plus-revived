...
public class Settings {
	...
	static {
		...
		if (!GraphicsEnvironment.isHeadless()) {
			int screenRefreshRate = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDisplayMode().getRefreshRate();

			if (screenRefreshRate != DisplayMode.REFRESH_RATE_UNKNOWN)
				initialFPS = screenRefreshRate;
		}

		options.put("fps", new RangeEntry("Max FPS", 10, 300, initialFPS));
