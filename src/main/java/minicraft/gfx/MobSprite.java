...
public class MobSprite extends Sprite {
	...
	*/
	...
	public static MobSprite[] compileSpriteList(int sheetX, int sheetY, int width, int height, int mirror, int number, SpriteSheet sheet) {
	...
	public static MobSprite[][] compileMobSpriteAnimations(int sheetX, int sheetY, SpriteSheet sheet) {
		...
		MobSprite[] set1 = MobSprite.compileSpriteList(sheetX, sheetY, 2, 2, 0, 4, sheet);

		// Contents: down 2, up 2, left 1, left 2
		MobSprite[] set2 = MobSprite.compileSpriteList(sheetX, sheetY, 2, 2, 1, 4, sheet);
	...
	public static MobSprite[][] compilePlayerSpriteAnimations(int sheetX, int sheetY, SpriteSheet sheet) {
		MobSprite[][] sprites = new MobSprite[4][2];
		// dir numbers: 0=down, 1=up, 2=left, 3=right.
		/// On the spritesheet, most mobs have 4 sprites there, first facing down, then up, then right 1, then right 2. The first two get flipped to animate them, but the last two get flipped to change direction.
	...
	public void renderRow(int r, Screen screen, int x, int y, boolean fullbright) {
		...
		for (int c = 0; c < row.length; c++) { // Loop across through each column
			screen.render(x + c * 8, y, row[c].x, row[c].y, row[c].mirror, row[c].sheet, -1, fullbright); // Render the sprite pixel.
		}
	}
}
