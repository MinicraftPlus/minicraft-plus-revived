		...
		if ((input.getKey("select").clicked || input.getKey("attack").clicked) && recipeMenu.getSelection() >= 0) {
			...
			if (selectedRecipe.getCanCraft()) {
				if (selectedRecipe.getProduct().equals(Items.get("Workbench"))){
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.benchmarking",true);
				}
				if (selectedRecipe.getProduct().equals(Items.get("Plank"))){
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.planks",true);
				}
				if (selectedRecipe.getProduct().equals(Items.get("Wood Door"))){
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.doors",true);
				}
				if (selectedRecipe.getProduct().equals(Items.get("Rock Sword")) ||
					...
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.upgrade", true);
				}
				if (selectedRecipe.getProduct().equals(Items.get("blue clothes")) ||
					...
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.clothes", true);
				}
