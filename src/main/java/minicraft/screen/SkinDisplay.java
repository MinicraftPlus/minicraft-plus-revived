...
public class SkinDisplay extends Display {
    ...
    private int step; // It is used to loop the animations
	...
	public void render(Screen screen) {
		...
		int spriteIndex = (step / 30) % 9; // 9 = 8 Frames for sprite
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (selected == 0) {
					screen.render(xo + x * 8, yo + y * 8, x + 2 * spriteIndex + (y + 16) * 32, 0, 2); // Paul
					
				} else if (selected == 1) {
					screen.render(xo + x * 8, yo + y * 8, x + 2 * spriteIndex + y * 32, 0, 4); // Paul (Cape)
					
				} else if (selected == 2) {
					screen.render(xo + x * 8, yo + y * 8, x + 2 * spriteIndex + (y + 4) * 32, 0, 4); // Familiar Boy
					
				} else if (selected == 3) {
					screen.render(xo + x * 8, yo + y * 8, x + 2 * spriteIndex + (y + 8) * 32, 0, 4); // Familiar Girl
					
				} else if (selected > 3) {
					screen.render(xo + x * 8, yo + y * 8, x + 2 * spriteIndex + y * 32, 0, 4); // Custom Skin
