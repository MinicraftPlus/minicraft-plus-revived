		...
		switch(inType) {
			...
			case TILES:
				...
				for (int i = 0; i < tiledata.length/2 && i < level.tiles.length; i++) {
					...
					level.data.NBTdata[i] = tiledata[i].right;
				}
			...
			case TILE:
				...
				try {
					theLevel.data.NBTdata[pos] = NBTReader.read(new ByteArrayInputStream(data[3].getBytes()));
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					theLevel.data.NBTdata[pos] = new NBTCompound();
				}
				return true;
