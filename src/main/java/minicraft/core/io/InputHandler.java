...
public class InputHandler implements KeyListener {
	...
	*/
	...
	private void initKeyMap() {
		...
		// Saves the game while still playing
		keymap.put("QUICKSAVE", "R"); 

		// Attack action references "C" key
		keymap.put("ATTACK", "C|SPACE|ENTER");
		
		// And so on... menu does various things.
		keymap.put("MENU", "X|E");
		
		// Open/close personal crafting window.
		keymap.put("CRAFT", "Z|SHIFT-E");
		keymap.put("PICKUP", "V|P");
		
		// Pickup torches / furniture; this replaces the power glove.
		keymap.put("DROP-ONE", "Q");
		
		// Drops the item in your hand, or selected in your inventory, by ones; it won't drop an entire stack
		keymap.put("DROP-STACK", "SHIFT-Q");
		...
		// Toggle potion effect display
		keymap.put("POTIONEFFECTS", "P"); 
		
		// Toggle fps display
		// keymap.put("FPSDISP", "F3"); 
		
		// Toggle player stats display
		keymap.put("INFO", "SHIFT-I"); 
	...
	public class Key {
		...
		public void toggle(boolean pressed) {
			...
			// Set down to the passed in value; the if statement is probably unnecessary...
			down = pressed; 
	...
	private Key getKey(String keytext, boolean getFromMap) {
		...
		synchronized ("lock") {
			...
			// Gets the key object from keyboard, if if exists.
