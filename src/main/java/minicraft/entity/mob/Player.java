		...
		if (activeItem != null) {
			...
			if (activeItem instanceof ToolItem && stamina - 1 >= 0) {
				...
				if (tool.type == ToolType.Bow && tool.dur > 0 && inventory.count(Items.arrowItem) > 0) {
					...
					if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.bow",true);
				...
				if (random.nextInt(100) + 1 <= chance) {
					...
					if (itemData.startsWith(";")) {
						...
						if (Items.get(itemData).equals(Items.get("Raw Fish"))) {
							if (!Game.isMode("creative")) AchievementsDisplay.setAchievement("minicraft.achievement.fish",true);
						}
