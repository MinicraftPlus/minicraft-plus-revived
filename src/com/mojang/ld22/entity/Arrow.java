...
public class Arrow extends Entity {
 ...
 public void tick() {
  ...
  /*
   if (level.getTile(x/16, y/16) != Tile.stairsUp ) {
	   if (level.getTile(x/16, y/16) != Tile.stairsDown) {
		 if (damage > 3){
		 ...
		 if (ModeMenu.creative == true) {
		level.setTile(x/16, y/16, Tile.wheat, 0);
		}
		}
	   }
   }
  ...
  if (level.getTile(x/16, y/16).mayPass(level, x/16, y/16, this) || level.getTile(x/16, y/16).connectsToWater|| level.getTile(x/16, y/16).id == 16) {
  } else {
	this.remove();
