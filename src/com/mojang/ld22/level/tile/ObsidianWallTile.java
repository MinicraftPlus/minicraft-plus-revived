...
public class ObsidianWallTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		if (!d && !l) {
			if (!dl)
				...
				screen.render(x * 16 + 0, y * 16 + 8, 7 + 25 * 32, backColor, 3);
		} else
			screen.render(x * 16 + 0, y * 16 + 8, (l ? 6 : 5) + (d ? 25 : 26) * 32, transitionColor, 3);
		if (!d && !r) {
			if (!dr)
				screen.render(x * 16 + 8, y * 16 + 8, 7 + 2 * 32, col, 0);
			else
				screen.render(x * 16 + 8, y * 16 + 8, 8 + 25 * 32, backColor, 3);
		} else
	...
	public boolean interact(Level level, int xt, int yt, Player player, Item item, int attackDir) {
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.pickaxe && tool.level > 2 && AirWizard.healthstat < 1) {// This makes it so that you can only break obsidian walls with a gem pickaxe and only after you beat the air wizard
