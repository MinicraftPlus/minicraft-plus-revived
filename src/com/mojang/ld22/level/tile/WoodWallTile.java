	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		if (!d && !l) {
			if (!dl)
				...
				screen.render(x * 16 + 0, y * 16 + 8, 7 + 23 * 32, transitionColor, 3);
		} else
			screen.render(x * 16 + 0, y * 16 + 8, (l ? 6 : 5) + (d ? 22 : 23) * 32, transitionColor, 3);
		if (!d && !r) {
			if (!dr)
				screen.render(x * 16 + 8, y * 16 + 8, 5 + 23 * 32, col, 0);
			else
				screen.render(x * 16 + 8, y * 16 + 8, 8 + 23 * 32, transitionColor, 3);
		} else
	...
	public void hurt(Level level, int x, int y, int dmg) {
		...
		if (damage >= wwH) {
			...
			for (int i = 0; i < count; i++) {
				level.add(new ItemEntity(new ResourceItem(Resource.plank), x * 16 + random.nextInt(10) + 3, y * 16 + random.nextInt(10) + 3));
			}
