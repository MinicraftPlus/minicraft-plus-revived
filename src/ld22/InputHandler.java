...
public class InputHandler implements MouseListener, KeyListener {
	...
	public InputHandler(Game game) {
		keymap =
				new HashMap<String, String>(); //stores custom key name with physical key name in keyboard.
	...
	public void tick() {
		...
		for (int i = 0; i < keys.length; i++) keys[i].tick();
	}
	...
	public Key getKey(String keytext) {
		...
		if (keyboard.containsKey(keytext)) key = keyboard.get(keytext);
		else {
			key = new Key();
