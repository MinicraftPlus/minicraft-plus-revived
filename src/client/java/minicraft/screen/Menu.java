...
public class Menu {
	...
	ListEntry[] getEntries() { return entries.stream().map(a -> a.delegate).toArray(ListEntry[]::new); }
	...
	@Nullable ListEntry getCurEntry() { return entries.size() == 0 ? null : entries.get(selection).delegate; }
		...
		if(title.length() > 0) {
			if (drawVertically) {
				for (int i = 0; i < title.length(); i++) {
					if (hasFrame) screen.render(null, titleLoc.x, titleLoc.y + i * Font.textHeight(), 3, 6, 0, hudSheet.getSheet());
					Font.draw(title.substring(i, i + 1), screen, titleLoc.x, titleLoc.y + i * Font.textHeight(), titleColor);
				}
			} else {
				for (int i = 0; i < title.length(); i++) {
					if (hasFrame) screen.render(null, titleLoc.x + i * Font.textWidth(" "), titleLoc.y, 3, 6, 0, hudSheet.getSheet());
