...
//	public static class Frame extends Rectangle {
//		private static final LinkedSprite hudSheet = new LinkedSprite(SpriteType.Gui, "hud");
//
//		private final Builder builder = new Builder(); // Acts as a frame updater.
//		private boolean renderFrame = false;
//		private Rectangle entryBounds = null;
//		private Rectangle renderingBounds = null;
//
//		private boolean drawVertically = false;
//		private RelPos titlePos = RelPos.LEFT;
//
...
//			entryBounds.translate(xoff, yoff);
//			renderingBounds.translate(xoff, yoff);
//		}
//
//		public void render(Screen screen) {
//			if (renderFrame) {
//				int bottom = getBottom() - MinicraftImage.boxWidth;
//				int right = getRight() - MinicraftImage.boxWidth;
//
//				for (int y = getTop(); y <= bottom; y += MinicraftImage.boxWidth) { // loop through the height of the bounds
//					for (int x = getLeft(); x <= right; x += MinicraftImage.boxWidth) { // loop through the width of the bounds
//
//						boolean xend = x == getLeft() || x == right;
//						boolean yend = y == getTop() || y == bottom;
//						int spriteoffset = (xend && yend ? 0 : (yend ? 1 : (xend ? 2 : 3))); // determines which sprite to use
//						int mirrors = (x == right ? 1 : 0) + (y == bottom ? 2 : 0); // gets mirroring
//
//						screen.render(x, y, spriteoffset, 6, mirrors, hudSheet.getSheet());
...
//
//				drawVertically = titlePos == RelPos.LEFT || titlePos == RelPos.RIGHT;
//
...
//				menu.frame = new Frame(menuPos.positionRect(menuSize, anchor, new Rectangle())); // reset to a value that is actually useful to the menu
//				menu.frame.renderFrame = hasFrame;
//
//				if (limitingBounds != null) {
//					// Exceeded areas are cut off from the bounds.
//					if (!limitingBounds.intersects(menu.frame))
//						throw new IndexOutOfBoundsException("limitingBounds does not intersect bounds");
//					int x = Math.max(limitingBounds.getLeft(), menu.frame.getLeft()); // Get the rightmost left bound
//					int y = Math.max(limitingBounds.getTop(), menu.frame.getTop()); // Get the bottommost top bound
//					int x1 = Math.min(limitingBounds.getBottom(), menu.frame.getBottom()); // Get the topmost bottom bound
//					int y1 = Math.min(limitingBounds.getRight(), menu.frame.getRight()); // Get the leftmost right bount
//					menu.frame = new Frame(new Rectangle(x, y, x1, y1, Rectangle.CORNERS)); // Reset to the bounds after cut off the exceeded bounds
//				}
//
//				menu.frame.entryBounds = border.subtractFrom(menu.frame);
//
//				menu.titleLoc = titlePos.positionRect(titleDim, menu.frame);
//
//				menu.frame.renderingBounds = renderingBounds == null ? menu.frame.entryBounds : renderingBounds;
//
...
//					else {
//						if (!setTitleColor) titleCol = hasFrame ? Color.YELLOW : Color.SILVER;
