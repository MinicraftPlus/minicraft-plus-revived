...
/**
 * Only the direct subclasses of {@link AppToast} and {@link GameToast} are used.
 */
public abstract class Toast {
	...
	/**
	 * Acts as a helper function in case some implementations may still use the standard texture formats.
 	 */
	protected final void renderFrame(Screen screen, int x, int y, int w, int h, Sprite sprite) {
		for (int i = 0; i < w; ++i) {
			for (int j = 0; j < h; ++j) {
