...
public class LevelGen {
    ...
    public LevelGen(int w, int h, int featureSize) {
        this(0, 0, w, h, featureSize, 0);
    }

    public LevelGen(int w, int h, int featureSize, int layer) {
        this(0, 0, w, h, featureSize, layer);
    }

    public LevelGen(int xOffset, int yOffset, int w, int h, int featureSize) {
        this(xOffset, yOffset, w, h, featureSize, 0);
    }
    ...
    private static void generateSkyChunk(ChunkManager map, int chunkX, int chunkY) {
        ...
        for (int i = 0; i < S * S / 50; i++) {
            ...
            for (int yy = y - 1; yy <= y + 1; yy++) {
                for (int xx = x - 1; xx <= x + 1; xx++) {
                    if (map.getTile(xx, yy) == Tiles.get("Infinite Fall")) {
                        continue stairsLoop;
                    }
                }
            }
        ...
        for (int i = 0; i < S * S; i++) {
            ...
            for (int yy = y - 1; yy <= y + 1; yy++) {
                for (int xx = x - 1; xx <= x + 1; xx++) {
                    if (map.getTile(xx, yy) != Tiles.get("cloud")) {
                        continue stairsLoop;
                    }
                }
            }
        ...
        while (true) {
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    ...
                    if (map.getTile(x, y) == Tiles.get("water")) {
                        pixels[i] = 0x000080;
                    }
                    if (map.getTile(x, y) == Tiles.get("iron Ore")) {
                        pixels[i] = 0x000080;
                    }
                    if (map.getTile(x, y) == Tiles.get("gold Ore")) {
                        pixels[i] = 0x000080;
                    }
                    if (map.getTile(x, y) == Tiles.get("gem Ore")) {
                        pixels[i] = 0x000080;
                    }
                    if (map.getTile(x, y) == Tiles.get("grass")) {
                        pixels[i] = 0x208020;
                    }
                    if (map.getTile(x, y) == Tiles.get("rock")) {
                        pixels[i] = 0xa0a0a0;
                    }
                    if (map.getTile(x, y) == Tiles.get("dirt")) {
                        pixels[i] = 0x604040;
                    }
                    if (map.getTile(x, y) == Tiles.get("sand")) {
                        pixels[i] = 0xa0a040;
                    }
                    if (map.getTile(x, y) == Tiles.get("Stone Bricks")) {
                        pixels[i] = 0xa0a040;
                    }
                    if (map.getTile(x, y) == Tiles.get("tree")) {
                        pixels[i] = 0x003000;
                    }
                    if (map.getTile(x, y) == Tiles.get("flower")) {
                        pixels[i] = 0xa050ff;
                    }
                    if (map.getTile(x, y) == Tiles.get("cactus")) {
                        pixels[i] = 0x105000;
                    }
                    if (map.getTile(x, y) == Tiles.get("Obsidian Wall")) {
                        pixels[i] = 0x0aa0a0;
                    }
                    if (map.getTile(x, y) == Tiles.get("Obsidian")) {
                        pixels[i] = 0x000000;
                    }
                    if (map.getTile(x, y) == Tiles.get("lava")) {
                        pixels[i] = 0xffff2020;
                    }
                    if (map.getTile(x, y) == Tiles.get("cloud")) {
                        pixels[i] = 0xa0a0a0;
                    }
                    if (map.getTile(x, y) == Tiles.get("Stairs Down")) {
                        pixels[i] = 0xff0000ff;
                    }
                    if (map.getTile(x, y) == Tiles.get("Stairs Up")) {
                        pixels[i] = 0xff00ffff;
                    }
                    if (map.getTile(x, y) == Tiles.get("Cloud Cactus")) {
                        pixels[i] = 0xffff00ff;
                    }
                    if (map.getTile(x, y) == Tiles.get("Ornate Obsidian")) {
                        pixels[i] = 0x000f0a;
                    }
                    if (map.getTile(x, y) == Tiles.get("Raw Obsidian")) {
                        pixels[i] = 0x0a0080;
                    }
                    if (map.getTile(x, y) == Tiles.get("Lava Stone")) {
                        pixels[i] = 0xffa500;
                    }
            ...
            int op = JOptionPane.showOptionDialog(null, null, "LevelViewer " + Game.VERSION + " | Seed: " + worldSeed + " | Level: " + lvl, JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE,
                    new ImageIcon(img.getScaledInstance(512, 512, Image.SCALE_AREA_AVERAGING)),
